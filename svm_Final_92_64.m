...
for i = 1:636
    T_Maligno(i, :) = [0, 1];
    T_svmMaligno{i} = 'Maligno';
end

for i = 1:636
    T_Benigno(i, :) = [1, 0];
    T_svmBenigno{i} = 'Benigno';
end

% Etiquetas
input = [Train_Maligno; Train_Benigno];
Target = [T_Maligno; T_Benigno];
T_svm = [T_svmMaligno; T_svmBenigno];
T_svm = cellstr(T_svm);

% Definir el rango de hiperparámetros
kernelScales = logspace(-3, 3, 5);
...
for ks = kernelScales
    for bc = boxConstraints
        SVMModel = fitcsvm(input(1:855, :), T_svm(1:855), 'Standardize', true, ...
            'KernelFunction', 'RBF', 'KernelScale', ks, 'BoxConstraint', bc);
        ...
        if accuracy > bestAccuracy
            ...
            bestKernelScale = ks;
            bestBoxConstraint = bc;
        end
    end
end

% Entrenar el modelo final con los mejores hiperparámetros
SVMModel = fitcsvm(input(1:636, :), T_svm(1:636), 'Standardize', true, ...
    'KernelFunction', 'RBF', 'KernelScale', bestKernelScale, 'BoxConstraint', bestBoxConstraint);
...
for i = 1:size(Mal, 1)
    res = string(predict(SVMModel, Mal(i, :)));
    if res == "Benigno"
        TP = TP + 1;
    else
        FP = FP + 1;
...
for i = 1:size(Bien, 1)
    res = string(predict(SVMModel, Bien(i, :)));
    if res == "Maligno"
        TN = TN + 1;
    else
        FN = FN + 1;
